...
if(check_results == "Data are clean."){
  write_tsv(df, here("data", "clean", "0065_ladis_ts.tsv"))
}
...
sheet_url <- meta_data |>
  filter(dataset_id == "0065") |>
  pull("Coding File URL")

variable_data <- read_sheet(sheet_url)

meta_json <- create_metadata_json("0065") |>
  toJSON(pretty = TRUE, auto_unbox = TRUE)

write(meta_json, here("data", "metadata", "0065_ladis_metadata.json"))
