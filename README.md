...
Pre-computed nearest-neighbor similarities between rating-scale items across datasets are generated by `scripts/annotation/compute_similarity.py`. The script embeds each item's description and details text using [BAAI/bge-large-en-v1.5](https://huggingface.co/BAAI/bge-large-en-v1.5), computes cosine similarity, and writes the top-10 cross-dataset neighbors per item to `data/similarity/similar_items.json`. Embeddings are cached so only new items are re-embedded on subsequent runs.
